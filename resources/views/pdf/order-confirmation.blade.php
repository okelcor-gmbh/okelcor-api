...
<div class="page">
    ...
    {{-- Document title --}}
    <div class="doc-title">
        Order Confirmation {{ $document->number }}
        @if ($quote?->ref_number) by proposal {{ $quote->ref_number }}@endif
    </div>

    <p class="intro-p">Dear customer,</p>
    <p class="intro-p" style="margin-bottom:16px;">
        Please find below the order confirmation for order {{ $document->number }}:
    </p>
