...
XML;
    ...
    public function test_adopting_the_ebay_price_updates_the_website_without_calling_ebay(): void
    {
        $product = $this->listedProduct(['sku' => 'ADOPT-1', 'price' => 100, 'cost_price' => 60]);

        DB::table('ebay_live_listings')->insert([
            'sku' => 'ADOPT-1', 'status' => 'published', 'price' => 84.90, 'currency' => 'EUR',
            'quantity' => 4, 'fetched_at' => now(), 'created_at' => now(), 'updated_at' => now(),
        ]);

        // No EbaySellingService mock on purpose: adoption must never touch
        // eBay — eBay already shows this price.
        $this->actingAs($this->admin(), 'sanctum')
            ->postJson("/api/v1/admin/ebay/audit/{$product->id}/adopt-ebay-price")
            ->assertOk()
            ->assertJsonPath('data.price', 84.9)
            ->assertJsonPath('data.old_price', 100);

        $this->assertSame(84.90, (float) $product->fresh()->price);
        $this->assertDatabaseHas('ebay_listing_logs', [
            'sku'    => 'ADOPT-1',
    ...
    public function test_adoption_refuses_without_a_live_price_or_with_foreign_currency(): void
    {
        $noSnapshot = $this->listedProduct(['sku' => 'NOSNAP-1', 'price' => 100]);

        $this->actingAs($this->admin(), 'sanctum')
            ->postJson("/api/v1/admin/ebay/audit/{$noSnapshot->id}/adopt-ebay-price")
            ->assertStatus(422);
        $this->assertSame(100.0, (float) $noSnapshot->fresh()->price);

        $gbp = $this->listedProduct(['sku' => 'GBP-1', 'price' => 100]);
        DB::table('ebay_live_listings')->insert([
            'sku' => 'GBP-1', 'status' => 'published', 'price' => 70, 'currency' => 'GBP',
            'quantity' => 1, 'fetched_at' => now(), 'created_at' => now(), 'updated_at' => now(),
        ]);

        $this->actingAs($this->admin(), 'sanctum')
            ->postJson("/api/v1/admin/ebay/audit/{$gbp->id}/adopt-ebay-price")
            ->assertStatus(422);
        $this->assertSame(100.0, (float) $gbp->fresh()->price);
    }

    public function test_bulk_adoption_sweeps_every_drifted_product_and_reports_each_change(): void
    {
        $drifted   = $this->listedProduct(['sku' => 'BULK-1', 'price' => 100]);
        $matching  = $this->listedProduct(['sku' => 'BULK-2', 'price' => 79.90]);
        $noLiveRow = $this->listedProduct(['sku' => 'BULK-3', 'price' => 50]);

        DB::table('ebay_live_listings')->insert([
            ['sku' => 'BULK-1', 'status' => 'published', 'price' => 89.90, 'currency' => 'EUR',
             'quantity' => 2, 'fetched_at' => now(), 'created_at' => now(), 'updated_at' => now()],
            ['sku' => 'BULK-2', 'status' => 'published', 'price' => 79.90, 'currency' => 'EUR',
             'quantity' => 2, 'fetched_at' => now(), 'created_at' => now(), 'updated_at' => now()],
        ]);

        $response = $this->actingAs($this->admin(), 'sanctum')
            ->postJson('/api/v1/admin/ebay/audit/adopt-ebay-prices', ['all_drifted' => true])
            ->assertOk();

        $this->assertSame(1, $response->json('meta.updated_count'));
        $this->assertSame('BULK-1', $response->json('data.updated.0.sku'));
        $this->assertSame(89.90, (float) $drifted->fresh()->price);
        $this->assertSame(79.90, (float) $matching->fresh()->price);
        $this->assertSame(50.0, (float) $noLiveRow->fresh()->price);
    }

    public function test_bulk_adoption_by_ids_reports_the_rows_it_could_not_change(): void
    {
        $drifted = $this->listedProduct(['sku' => 'PICK-1', 'price' => 100]);
        $ghost   = $this->listedProduct(['sku' => 'PICK-2', 'price' => 60]);

        DB::table('ebay_live_listings')->insert([
            'sku' => 'PICK-1', 'status' => 'published', 'price' => 92.50, 'currency' => 'EUR',
            'quantity' => 1, 'fetched_at' => now(), 'created_at' => now(), 'updated_at' => now(),
        ]);

        $response = $this->actingAs($this->admin(), 'sanctum')
            ->postJson('/api/v1/admin/ebay/audit/adopt-ebay-prices', ['ids' => [$drifted->id, $ghost->id]])
            ->assertOk();

        $this->assertSame(1, $response->json('meta.updated_count'));
        $this->assertSame(1, $response->json('meta.skipped_count'));
        $this->assertSame('PICK-2', $response->json('data.skipped.0.sku'));
        $this->assertSame(92.50, (float) $drifted->fresh()->price);
        $this->assertSame(60.0, (float) $ghost->fresh()->price);
    }

    public function test_only_ebay_manage_roles_can_adopt_prices(): void
    {
        $product = $this->listedProduct(['sku' => 'PERM-1', 'price' => 100]);
