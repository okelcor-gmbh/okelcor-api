...
class AdminMarketingContactController extends Controller
{
    ...
    {
        // A contact lives in exactly one market (one `market` column, unique
        // email), so re-adding an existing address under a different market is
        // really a *move* request. Answered before validation so the response
        // can say which market already holds it and hand back its id — a bare
        // "email already exists" left the marketing team with no next step but
        // to delete the contact and re-add it. Still a 422 with `errors.email`
        // populated, so any client that only reads the standard validation
        ...
        if ($request->filled('email')) {
            ...
            if ($existing) {
                $target = $request->filled('market') ? $this->normalizeMarket($request->input('market')) : null;

                return response()->json([
                    'message' => $target && $target !== $existing->market
                        ? "This contact is already in the \"{$existing->market}\" market. Move it to \"{$target}\" instead of adding it again."
                        : "This contact is already in the \"{$existing->market}\" market.",
                    'errors'  => ['email' => ['This email is already on the marketing list.']],
                    'code'    => 'contact_exists',
                    'data'    => [
                        'existing_contact' => $this->formatContact($existing),
                        'can_move'         => $target !== null && $target !== $existing->market,
