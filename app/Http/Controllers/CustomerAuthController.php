...
class CustomerAuthController extends Controller
{
     ...
     *
     * Named rather than inline because it is now load-bearing twice over:
     * completing a reset both sets a password and confirms the email address.
     */
    public const RESET_TOKEN_TTL_MINUTES = 60;
    ...
    public function resetPassword(Request $request): JsonResponse
    {
        ...
        // Expiry — and it has never worked.
        //
        // This was `now()->diffInMinutes($record->created_at) > 60`. Carbon 3
        // (Laravel 11) returns a SIGNED difference, so for a token created in
        // the past that expression is negative — a three-hour-old token gives
        // -180, and -180 > 60 is false. Every password reset link this system
        // has ever sent has been valid forever, and an old one sitting in an
        // inbox or a mail archive still works today.
        //
        // Found by a test written for something else, which is the only reason
        // it was ever going to be found: nothing about the broken version fails
        // visibly. It matters more from this commit onward, because completing
        // a reset now also confirms the email address.
        //
        // Expressed as "created_at plus the window is in the past" so there is
        // no sign to get wrong.
        $createdAt = Carbon::parse($record->created_at);

        if ($createdAt->copy()->addMinutes(self::RESET_TOKEN_TTL_MINUTES)->isPast()) {
        ...
        // Completing a password reset IS proof the customer controls the
        // address, so it verifies the address.
        //
        // This used to happen only for 'invited' customers, and the omission
        // trapped people. A buyer whose account was approved but never verified
        // is told at login to verify their email; the link from registration
        // expired 24 hours after they signed up; the only self-service action
        // left to them is a password reset — which did not count, so they came
        // back to the same wall. Reported by a customer who had been round that
        // loop several times: "I have completed a few of these verifications
        // that prompt a password change, but the login still prompts me to
        // verify my email again."
        //
        // The token was generated by us, mailed to that address, and returned
        // by whoever opened it. That is the same evidence a verification link
        // collects, by the same mechanism. Gating it on onboarding status
        // confused who the customer is with what they had just proved.
        //
        // An address verified earlier keeps its original date — this records
        // when the mailbox was first proved, not when it was last used.
        ...
        if ($customer->email_verified_at === null) {
            SecurityEventService::log(
                'email_verified', $customer->id, null, null,
                'Email verified by completing a password reset', 'info'
            );
        }
    ...
    // -------------------------------------------------------------------------
    /**
     * Link building and sending now live in CustomerEmailVerifier — an admin
     * approving a buyer needs to send the same mail, and a private method here
     * meant that flow could not, which is how the approval email ended up
     * telling customers to verify an address with no link to do it with.
     */
    private function sendVerificationEmail(Customer $customer): void
    {
        app(CustomerEmailVerifier::class)->send($customer);
    }
